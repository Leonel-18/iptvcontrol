...
import { ArrowLeft, ExternalLink, PauseCircle, PlayCircle, Plus, UserX } from 'lucide-react';
...
import type { AccountDevice, CustomerDetail as CustomerDetailData } from '@/lib/types';
...
export const CustomerDetail = () => {
  ...
  const [accion, setAccion] = useState<'suspend' | 'reactivate' | 'terminate' | null>(null);
  ...
  const transicion = useMutation({
    ...
    onSuccess: (_resultado, tipo) => {
      const mensajes = {
        suspend: 'Cliente suspendido. Su dispositivo queda reservado y no se puede reasignar.',
        reactivate: 'Cliente reactivado. Su dispositivo volvió a habilitarse en el proveedor.',
        terminate: 'Cliente dado de baja. Su dispositivo quedó disponible para reasignar.',
      } as const;
    ...
    <>
      ...
      <PageHeader
        ...
        acciones={
          <>
            ...
            {!esOperador && activo ? (
              <>
                <Button variant="secondary" size="sm" onClick={() => setAgregarDispositivo(true)}>
                  <Plus />
                  Agregar dispositivo
                </Button>
                <Button variant="secondary" size="sm" onClick={() => setAccion('suspend')}>
                  <PauseCircle />
                  Suspender
                </Button>
                <Button variant="danger" size="sm" onClick={() => setAccion('terminate')}>
                  <UserX />
                  Dar de baja
                </Button>
              </>
            ...
            {!esOperador && suspendido ? (
              <>
                <Button variant="primary" size="sm" onClick={() => setAccion('reactivate')}>
                  <PlayCircle />
                  Reactivar
                </Button>
                <Button variant="danger" size="sm" onClick={() => setAccion('terminate')}>
      ...
      {suspendido ? (
        <Alert tone="warning" titulo="Cliente suspendido" className="mb-4">
          Su dispositivo está liberado en el proveedor pero reservado: nadie más puede tomarlo. La
          única forma de liberarlo para otro cliente es pasarlo a baja definitiva.
        </Alert>
      ) : null}

      {dadoDeBaja ? (
        <Alert tone="info" titulo="Cliente dado de baja" className="mb-4">
          Sus dispositivos quedaron disponibles y pueden asignarse a un cliente nuevo desde la
        ...
        <div className="space-y-4 lg:col-span-2">
          <Card>
            <CardHeader className="flex-row items-center justify-between gap-2">
              ...
              {!esOperador && activo ? (
                <Button variant="ghost" size="sm" onClick={() => setAgregarDispositivo(true)}>
                  <Plus />
                  Agregar
                </Button>
              ) : null}
                        ...
                        {!esOperador ? (
                          <TD>
                            <div className="flex flex-col gap-0.5">
                              {dispositivo.mac ? (
                                <span className="id-tecnico">{formatearMac(dispositivo.mac)}</span>
                              ) : null}
            ...
            <Card>
              ...
              <CardContent>
                <dl className="divide-y divide-[rgb(var(--borde))]">
                  <DetailRow etiqueta="Nombre">{data.nombre_completo || '—'}</DetailRow>
                  <DetailRow etiqueta="Teléfono">{data.telefono || '—'}</DetailRow>
                  ...
                  <DetailRow etiqueta="ID en su gestión">
                    {data.id_gestion_externo ? (
                      <CopyableId valor={data.id_gestion_externo} etiqueta="ID de gestión" />
                    ) : (
                      '—'
                    )}
                  </DetailRow>
                  ...
                  {data.dado_de_baja_en ? (
                    <DetailRow etiqueta="Baja">{formatearFecha(data.dado_de_baja_en)}</DetailRow>
                  ) : null}
                </dl>
              </CardContent>
            </Card>
              ...
              <CardContent className="space-y-3">
                <dl className="space-y-2.5">
                  <div>
                    ...
                    <dd className="mt-0.5">
                      <CopyableId valor={data.cuenta.usuario} etiqueta="Usuario" />
                    </dd>
                  </div>
                  <div>
                    <dt className="eyebrow">Contraseña</dt>
                    <dd className="mt-0.5">
                      <SecretValue valor={data.cuenta.password} etiqueta="Contraseña" />
                    </dd>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <dd className="mt-0.5 font-mono text-xs">
                      Fijos {data.cuenta.fijos ?? '—'} · Móviles {data.cuenta.moviles ?? '—'}
                    </dd>
                  </div>
                </dl>
                ...
                {!data.cuenta.es_exclusiva ? (
                  <Alert tone="info">
                    Cuenta compartida: estas credenciales las usan también los otros clientes
                    finales que tengan un dispositivo en esta cuenta.
                  </Alert>
                ) : null}

                {data.cuenta.cerca_del_tope ? (
                  <Alert tone="warning">
                    Esta cuenta está cerca del tope. El próximo alta puede requerir otra cuenta.
                  </Alert>
                ) : null}
            ...
            <Card>
              ...
              <CardContent>
                <p className="text-sm texto-suave">
                  Los datos de contacto del cliente final y las credenciales de su cuenta los
                  administra la empresa revendedora. Para soporte, identifique el caso por el número
                  de cliente y el ID de la cuenta.
                </p>
              </CardContent>
            </Card>
          ) : null}
      ...
      >
        <Alert tone="warning">
          Mientras esté suspendido, ese dispositivo no se puede asignar a otro cliente. Para
          liberarlo hay que pasar este cliente a baja definitiva.
        </Alert>
      </ConfirmDialog>
      ...
      <ConfirmDialog
        ...
        descripcion="Se elimina el dispositivo en el proveedor y se resta un dispositivo habilitado de la cuenta."
      ...
      >
        <Alert tone="danger">
          El dispositivo queda disponible para otro cliente, que va a recibir las mismas
          credenciales de esta cuenta. La contraseña no se rota.
        </Alert>
      </ConfirmDialog>
