...
describe("utilidades de formato", () => {
  describe("cn", () => {
    ...
    it("descarta valores falsos", () => {
      const oculto = false;
      expect(cn("base", oculto && "oculto", undefined, "visible")).toBe(
        "base visible",
      );
    });
  });
  ...
  describe("formatearFecha", () => {
    it("usa el formato argentino dd/mm/aaaa", () => {
      expect(formatearFecha("2026-08-14T10:00:00.000Z")).toMatch(
        /^\d{2}\/\d{2}\/\d{4}$/,
      );
    });
  ...
  describe("formatearMac", () => {
    ...
    it("no intenta formatear identificadores largos del Proveedor", () => {
      expect(formatearMac("0123456789012345678901234567890123456789")).toBe(
