...
// Alturas crecientes para tres cupos comerciales 1+1.
const ALTURAS_BARRA = ["h-1.5", "h-2", "h-2.5", "h-3", "h-3.5", "h-4"] as const;

/** Ocupación comercial de una Cuenta, expresada en cupos 1+1. */
export type OcupacionCuenta = AccountCapacity;

/**
 * Medidor de capacidad de una Cuenta.
 *
 * La cantidad de segmentos representa los tres cupos 1+1. Los segmentos retoman
...
export const CapacityMeter = ({
  ...
  const bloqueadosVisibles = Math.min(
      ...
      >
        <span className="inline-flex items-end gap-[3px]" aria-hidden="true">
          {Array.from({ length: cantidadBarras }).map((_, indice) => {
            ...
            const esBloqueado =
              ocupado && indice >= capacidad.ocupados - bloqueadosVisibles;
            const altura =
              ALTURAS_BARRA[
                Math.floor((indice / cantidadBarras) * ALTURAS_BARRA.length)
              ];
