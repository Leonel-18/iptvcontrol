...
export const entityLabels = {
  reseller: { singular: 'Empresa Revendedora', plural: 'Empresas Revendedoras' },
  ...
  commercialPlan: { singular: 'Modalidad comercial', plural: 'Modalidades comerciales' },
  provider: { singular: 'Proveedor', plural: 'Proveedores' },
  teamMember: { singular: 'Miembro del equipo', plural: 'Equipo' },
  auditLog: { singular: 'Registro de auditoría', plural: 'Registro de auditoría' },
  operator: { singular: 'Operador Principal', plural: 'Operadores Principales' },
} as const;
...
export const deviceStatusHelp: Record<string, string> = {
  ...
  disponible: 'Liberado por una baja definitiva. Se puede asignar a un cliente nuevo.',
  dado_de_baja: 'Fuera de servicio.',
};

/** Tipos de Dispositivo (categorías con tope independiente de 3 cada una). */
...
export const customerIntakeLabels: Record<string, string> = {
  ...
  dispositivo_compartido: 'Solo un dispositivo (cuenta compartida)',
};

export const customerIntakeHelp: Record<string, string> = {
  cuenta_exclusiva:
    'Se crea una cuenta nueva sólo para este cliente. No la comparte con nadie.',
  dispositivo_compartido:
    'Se busca lugar en una cuenta propia con dispositivos disponibles. Si no hay, se crea una cuenta nueva.',
};
...
export const traducir = (diccionario: Record<string, string>, valor?: string | null): string => {
  if (!valor) return '—';
